...
export default function LandingPage() {
  ...
  const modules = [
    { title: "Módulo 1: A Base do Negócio", description: "Configuração de ferramentas, mentalidade de mercado e entendimento do potencial financeiro. Preparamos o terreno para você construir uma fonte de renda, não apenas aprender uma técnica." },
    { title: "Módulo 2: A Criação do Clone Perfeito", description: "O coração do método. O passo a passo detalhado para treinar a IA com as fotos do cliente, criando um 'ativo digital' hiper-realista que será a base para todos os ensaios." },
    { title: "Módulo 3: A Fábrica de Ensaios", description: "Domine a arte dos prompts para gerar imagens incríveis em qualquer cenário. Use nossos modelos prontos para entregar resultados que impressionam e garantem a satisfação do cliente." },
    { title: "Módulo 4: Estrutura de Vendas e Escala", description: "A técnica não vale nada sem vendas. Aprenda a empacotar, precificar e vender no Brasil (R$) e no exterior ($/€). Inclui modelos de mensagens e estratégias de prospecção." }
  ]
  ...
  return (
    <div className="min-h-screen bg-[#1c1a1d] text-gray-300 overflow-x-hidden font-sans">
              ...
              {testimonials.map((testimonial, index) => (
                <CarouselItem key={index} className="pl-8 md:basis-1/2 lg:basis-1/3">
                  <div className="bg-zinc-950 p-8 rounded-xl h-full flex flex-col border border-zinc-800">
                    <Quote className="w-8 h-8 text-pink-600 mb-4"/><p className="text-gray-300 flex-grow">"{testimonial.quote}"</p>
                    <div className="mt-6 pt-6 border-t border-zinc-800"><p className="font-semibold text-white">{testimonial.author}</p><p className="text-sm text-gray-500">{testimonial.role}</p></div>
                  </div>
                </CarouselItem>
              ))}
            </CarouselContent>
      ...
      {/* O Que é o Clone Perfeito? [RESTAURADO] */}
      ...
      {/* Módulos */}
      <section className="py-24 px-4 bg-[#1c1a1d]">
        <div className="max-w-4xl mx-auto"><div className="text-center mb-16"><BookOpen className="h-12 w-12 mx-auto text-gray-600 mb-4"/><h2 className="text-3xl font-bold text-white">A Estrutura Completa</h2></div>
          <Accordion type="single" collapsible className="w-full space-y-4">
            {modules.map((module, index) => (<AccordionItem key={index} value={`item-${index}`} className="bg-black/50 border border-zinc-800 rounded-lg"><AccordionTrigger className="text-left text-lg p-6 text-white hover:no-underline font-semibold"><span className="text-pink-500 mr-4">0{index+1}</span>{module.title}</AccordionTrigger><AccordionContent className="text-gray-400 text-base p-6 pt-0 pl-16">{module.description}</AccordionContent></AccordionItem>))}
          </Accordion>
        </div>
      </section>
      ...
      <footer className="py-12 px-4 text-center border-t border-zinc-900 bg-black">
        <div className="max-w-4xl mx-auto"><p className="text-gray-500 mb-2">© 2025 Clone Perfeito. Todos os direitos reservados.</p><p className="text-gray-600 text-sm mb-4">CNPJ: 50.243.188/2023-04 - GABRIEL MKT LTDA</p><div className="flex justify-center space-x-6 text-sm text-gray-500"><Link href="https://..." target="_blank" className="hover:text-white transition-colors">Política de Privacidade</Link><Link href="https://..." target="_blank" className="hover:text-white transition-colors">Termos de Uso</Link></div></div>
      </footer>
    </div>
  )
}
